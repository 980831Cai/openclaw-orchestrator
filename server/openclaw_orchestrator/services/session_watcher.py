...
class SessionWatcher:
    ...
    def _parse_line(
        ...
        role = str(envelope.get("role") or data.get("role") or "assistant")
        if role == "tool":
            role = "assistant"
        ...
        timestamp = envelope.get("timestamp") or data.get("timestamp") or ""
        metadata = (
    ...
    def mark_gateway_activity(self, payload: dict[str, Any]) -> None:
        ...
        content = self._coerce_content(message.get("content") or payload.get("content") or "")
