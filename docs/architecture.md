...
发送指令
  │
  ├→ ① Gateway RPC ── 成功 → 触发 Agent + 轮询 JSONL 等待响应
  │   sessions.spawn via JSON-RPC 2.0（创建会话 + 发送首条消息）
  │                  失败 ↓
...
if gateway_connector.connected:
    await gateway_connector.call_rpc("sessions.spawn", {
        "agentId": agent_id,
...
Gateway 是 OpenClaw 的 WebSocket 通信总线（`ws://localhost:18789`），使用 JSON-RPC 2.0 协议。**注意：Gateway 不是执行引擎，不支持 `agent.invoke`**。正确的方法是：
- **`sessions.spawn`**：创建新会话并发送首条消息，触发 Agent 执行
- **`sessions.send`**：向已有会话发送消息

Gateway spawn 成功后，仍需轮询 JSONL 等待 Agent 响应（Gateway 只负责消息投递，不等待 Agent 执行完成）。
