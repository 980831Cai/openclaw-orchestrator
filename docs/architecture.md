...
## 7. 创新四：Team Lead 编排体系

### 7.1 不只是标签

Team Lead 在 Orchestrator 中有实际的编排职责：

- **会议主持**：会议结束后由 Lead 阅读全部记录、撰写结论
- **结论权威**：Lead 的结论是最终决策，自动追加到 team.md
- **任务分配**（规划中）：接收到任务后由 Lead 分析并拆解子任务

### 7.2 设置方式

- UI 中每个团队成员 hover 显示"设为 Lead"按钮
- Lead 工位头顶浮动皇冠标识
- 工作室场景中视觉突出
...
（含 lead_agent_id / lead_mode /            agents/*/sessions/*.jsonl
 default_workflow_id / schedule_config）
workflows / executions                      agents/*/HEARTBEAT.md, skills.json
tasks / approvals                           openclaw.json（配置中枢）
（含 queue_status / parent_task_id /        teams/*/team.md, active/, archive/
 execution_id / blocked_reason 等）
meetings / notifications / knowledge        teams/*/governance/latest-report.json
schedule_jobs                               teams/*/governance/reports.jsonl
                                            cron/jobs.json
```
...
为支持“多团队并行 + 每个团队独立知识资产”，Orchestrator 在执行期引入统一的团队上下文装配器（`TeamContextService`），并升级为 **L1/L2/L3 分层补读策略**，严格约束会议信息读取边界。

### 12.1 设计目标

- **强隔离**：只允许读取 `teams/<team-id>/...` 下的团队文件，禁止跨团队内容混入。
- **分层补读**：
  - `L1`：任务目标 + 最新可执行交接；
  - `L2`：在依赖/阻塞/多上游时补读最近多条交接；
  - `L3`：兜底补读最近 N 条交接 + `task.md` 内授权决议摘要。
- **会议权限边界**：非参会 Agent 不读会议原始纪要；只可读后端写入 `task.md` 的授权决议摘要区。
- **预算可控**：通过统一字符预算和分层配额裁剪内容，避免提示词失控增长。
- **可观测**：执行日志记录“读层级 + 来源数量 + 预算占用 + 是否截断”。

### 12.2 执行链路接入

```text
workflow_engine._execute_task_node
  -> resolve_context_level(node, upstream)
  -> TeamContextService.build_context(team_id, task_id, read_level=L1/L2/L3)
  -> _build_task_prompt(..., team_context=...)

workflow_engine._execute_meeting_node
  -> 会议结束后仅写入 task.md#决议摘要区（授权摘要）
  -> 不向非参会 Agent 暴露会议原始纪要
```

### 12.3 回退策略

- 上下文加载失败（如团队范围校验失败、任务归属不匹配）时，记录 `warn` 日志并回退到原执行流程。
- 回退不阻断任务执行，保证系统可用性优先。
- 放行前交接校验可配置（`strict / lenient / disabled`），用于在可靠性与容错之间平衡。
