...
| Agent 形象 | SVG 代码绘制（非图片资源） | 确定性生成、零依赖、可随状态动态变化、打包体积极小 |
| Logo 设计 | 机械猫爪（Claw = 爪子） | 品牌名直接映射，科技感（电路线）+ 亲和力（猫爪）统一 |
...
支持半身模式（CartoonBody）：躯干 + 手臂 + 笔记本电脑（忙碌时） + ID 工牌。
...
- 电路纹理线（技术感）
- 顶部天线 + 绿色脉冲（在线指示）
- 4 种表情心情（happy / working / worried / waving）
- 用于：侧边栏、仪表盘欢迎区、所有空状态页面
...
### 10.5 cartoon-card 设计语言

统一的卡片视觉规范，全站所有交互面板使用同一基类：

```css
.cartoon-card {
  background: 渐变半透明（white 8% → 3%）
  border: 2px solid white/10，hover 时变为 purple/40
  border-radius: 16px
  box-shadow: 外阴影（深度感）+ 内发光（顶部白线）
  hover: 上浮 2px + 紫色边框 + 增强阴影
...
全局快捷面板（⌘K / Ctrl+K），参考 Linear/Raycast 的设计：
- 搜索范围：Agent / 工作室 / 页面导航
- 键盘导航：↑↓ 切换 + Enter 选择 + ESC 关闭
- 分组展示：按类别归组，每组有标题
- 毛玻璃蒙层 + 滑入动画
