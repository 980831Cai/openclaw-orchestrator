...
- **图遍历执行模型** — `while + current_node_id` 指针遍历，替代传统线性 for 循环
- **多节点类型** — Task（任务）、Condition（条件分支）、Parallel（并行）、Approval（审批）
- **条件分支回跳** — ConditionNode 的分支可指向上游节点，实现循环/回溯
- **审批暂停恢复** — ApprovalNode 暂停执行 → 人工审批 → 自动恢复后续流程
- **节点重试** — `maxRetries` + `retryDelayMs` 失败自动重试
- **防无限循环** — `maxIterations` 全局迭代次数守卫
- **上下文持久化** — `context_json` 序列化到数据库，支持跨重启恢复

### 👥 Agent 与 Team 管理

- Agent CRUD（身份配置、灵魂设定、规则、技能）
- Team 组建（成员管理、排班调度、共享知识库）
- Task 分配与制品管理
