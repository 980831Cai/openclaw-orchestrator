...
export function Sidebar({ expanded: expandedProp, onExpandedChange }: SidebarProps) {
  ...
  const localProcessOk = isGatewayRuntimeProcessRunning(gatewayRuntime)
  const gatewayReady = isGatewayRuntimeReady(gatewayRuntime)
  const runtimeActions = getGatewayRuntimeActions(gatewayRuntime, runtimeBusy)
  const allHealthy = realtimeOk && gatewayRpcOk && gatewayReady
