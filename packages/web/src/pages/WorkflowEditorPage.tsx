                    ...
                    {(selectedNode.data as WorkflowNodeData).type === 'approval' ? (
                      <>
                        ...
                        <p className="text-xs text-white/40">
                          {String((selectedNode.data as any).approvalMode || 'human') === 'agent'
                            ? '审批 Agent 只需返回 approve/reject 结果；无法解析时，这条审批会继续保留为待人工处理。'
                            : '人工审批会在通知中心生成待处理项，也可以通过审批接口继续流转。'}
                        </p>
                      </>
                    ) : null}
                      ...
                      <>
                        ...
                        {((selectedNode.data as any).joinMode || 'and') === 'xor' ? (
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-white/40">XOR 只允许这里选中的那条上游分支触发后续节点；其它分支到达时会被直接丢弃。</p>
                          </div>
                        ) : null}
