...
export function WorkflowEditorPage() {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex overflow-hidden">
      <div className="w-72 border-r border-white/5 flex flex-col bg-cyber-surface/30">
        <div className="p-4 border-b border-white/5 flex items-center justify-between gap-2">
          ...
          <Dialog>
            ...
            <DialogContent className="bg-cyber-panel border-white/10 text-white">
              <DialogHeader>
                <DialogTitle>创建工作流</DialogTitle>
              </DialogHeader>
              <div className="space-y-3">
                <Input value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="工作流名称" className="bg-cyber-bg border-white/10 text-white" />
                <Input value={newTeamId} onChange={(event) => setNewTeamId(event.target.value)} placeholder="teamId" className="bg-cyber-bg border-white/10 text-white" />
                <Button onClick={handleCreate} disabled={creating || !newName.trim()} className="w-full bg-gradient-to-r from-cyber-amber to-cyber-orange">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {workflows.map((workflow) => (
            <button
              ...
              className={cn('w-full flex items-center gap-2 p-3 rounded-xl transition-all cursor-pointer text-left', selected?.id === workflow.id ? 'bg-cyber-amber/15 border border-cyber-amber/30' : 'hover:bg-white/5 border border-transparent')}
            >
              <GitBranch className="w-4 h-4 text-cyber-amber/60 flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-white text-xs font-medium truncate">{workflow.name}</p>
                <p className="text-white/30 text-[10px]">{Object.keys(workflow.nodes).length} 个节点</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col">
        {!selected ? (
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <p className="text-white/30">选择或创建一个工作流开始编辑</p>
          </div>
        ) : (
            ...
            <div className="flex-1 h-full relative">
              ...
              >
                ...
                <Panel position="top-left" className="flex gap-2">
                  ...
                  <button onClick={() => addNode('join')} className="glass flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-white/60 hover:text-white hover:border-cyber-green/30 transition-all cursor-pointer">
                ...
                <Panel position="bottom-center" className="flex items-center gap-3 glass rounded-xl px-4 py-2">
                  ...
                  <Button size="sm" onClick={handleStop} disabled={!execution || execution.status !== 'running'} variant="destructive" className="h-8">
                  ...
                  {execution ? (
                    <span className={cn('text-[10px] px-2 py-0.5 rounded-full', execution.status === 'running' ? 'bg-cyber-green/20 text-cyber-green' : execution.status === 'completed' ? 'bg-cyber-blue/20 text-cyber-blue' : execution.status === 'failed' ? 'bg-red-500/20 text-red-300' : 'bg-white/10 text-white/50')}>
                      {execution.status}
                    </span>
                  ) : null}
                </Panel>
