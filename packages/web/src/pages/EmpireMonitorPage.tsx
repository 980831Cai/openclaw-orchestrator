import { useEffect, useMemo } from 'react'
import { useNavigate } from 'react-router-dom'
import {
  ActivityRankingBoard,
  DashboardHeroHeader,
  DashboardHudStats,
} from '@/components/empire-dashboard/HeroSections'
import {
  EmpireLiveFeed,
  EmpireRoomGrid,
} from '@/components/empire-dashboard/OpsSections'
import {
  buildHudStats,
  buildLiveFeed,
  buildRankedAgents,
  buildTeamRooms,
  resolveAgents,
  useNow,
  useNumberFormatter,
} from '@/components/empire-dashboard/model'
import { useAgents } from '@/hooks/use-agents'
import { useTeams } from '@/hooks/use-teams'
import { getActiveWorkflowCount, getActiveWorkflowSignals, getSchedulableWorkflows } from '@/lib/active-workflows'
import { useMonitorStore } from '@/stores/monitor-store'

export function EmpireMonitorPage() {
  const navigate = useNavigate()
  const { agents, fetchAgents } = useAgents()
  const { teams, fetchTeams } = useTeams()
  const {
    agentStatuses,
    events,
    gatewayConnected,
    gatewayRuntime,
    notifications,
    realtimeMessages,
    unreadCount,
    workflowSignals,
    scheduledWorkflows: workflowCatalog,
  } = useMonitorStore()

  useEffect(() => {
    fetchAgents()
    fetchTeams()
  }, [fetchAgents, fetchTeams])

  const resolvedAgents = useMemo(
    () =>
      resolveAgents(agents, agentStatuses, {
        events,
        messages: realtimeMessages,
        notifications,
        workflowSignals,
        gatewayConnected,
        gatewayRuntimeRunning: gatewayRuntime?.running === true,
      }),
    [agents, agentStatuses, events, notifications, realtimeMessages, workflowSignals, gatewayConnected, gatewayRuntime?.running],
  )

  const teamRooms = useMemo(
    () => buildTeamRooms(teams, resolvedAgents),
    [teams, resolvedAgents],
  )
  const rankedAgents = useMemo(
    () => buildRankedAgents(resolvedAgents),
    [resolvedAgents],
  )
  const workflowSignalList = useMemo(() => Array.from(workflowSignals.values()), [workflowSignals])
  const liveFeedItems = useMemo(
    () => buildLiveFeed(events, realtimeMessages, workflowSignalList, notifications),
    [events, notifications, realtimeMessages, workflowSignalList],
  )
  const activeWorkflowSignals = useMemo(() => getActiveWorkflowSignals(workflowSignalList), [workflowSignalList])
  const activeScheduledWorkflows = useMemo(() => getSchedulableWorkflows(workflowCatalog), [workflowCatalog])
  const activeWorkflowCount = useMemo(
    () => getActiveWorkflowCount({ signals: workflowSignalList, workflows: workflowCatalog }),
    [workflowSignalList, workflowCatalog],
  )

  const activeCount = resolvedAgents.filter(
    (agent) => agent.resolvedStatus === 'busy',
  ).length
  const hudStats = useMemo(
    () =>
      buildHudStats({
        agentCount: resolvedAgents.length,
        activeCount,
        roomCount: teamRooms.length,
        eventCount: events.length,
        gatewayRpcConnected: gatewayConnected,
        gatewayRuntimeRunning: gatewayRuntime?.running ?? false,
        messageCount: realtimeMessages.length,
        activeWorkflowCount,
      }),
    [
      activeCount,
      events.length,
      gatewayConnected,
      gatewayRuntime?.running,
      realtimeMessages.length,
      resolvedAgents.length,
      teamRooms.length,
      activeWorkflowCount,
    ],
  )

  const { date, time, briefing } = useNow()
  const numberFormatter = useNumberFormatter()

  return (
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(14,165,233,0.09),transparent_26%),radial-gradient(circle_at_80%_0%,rgba(168,85,247,0.1),transparent_22%),linear-gradient(180deg,rgba(15,23,42,0.96),rgba(2,6,23,1))] p-6 sm:p-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-5">
          <DashboardHeroHeader
            title="帝国态势面板"
            subtitle={`实时聚合 ${teamRooms.length} 个工作室、${resolvedAgents.length} 个 Agent、${activeWorkflowSignals.length} 条运行信号、${activeScheduledWorkflows.length} 个可触发定时与 ${liveFeedItems.length} 条近期动态`}
            time={time}
            date={date}
            briefing={briefing}
            reviewQueue={unreadCount}
            primaryCtaLabel="打开通信频道"
            primaryCtaDescription="从这里跳到通信频道，直接和单个 Agent 对话或查看会话细节。"
            onPrimaryCtaClick={() => navigate('/chat')}
          />

          <DashboardHudStats
            hudStats={hudStats}
            numberFormatter={numberFormatter}
          />

          <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(360px,0.95fr)]">
            <div className="space-y-5">
              <EmpireRoomGrid
                rooms={teamRooms}
                onOpenRoom={(roomId) => navigate(`/teams/${roomId}`)}
                onOpenAgent={(agentId) => navigate(`/chat?agent=${encodeURIComponent(agentId)}`)}
              />
            </div>

            <div className="space-y-5">
              <ActivityRankingBoard
                agents={rankedAgents}
                numberFormatter={numberFormatter}
              />
              <EmpireLiveFeed
                items={liveFeedItems}
                gatewayRpcConnected={gatewayConnected}
                gatewayRuntimeRunning={gatewayRuntime?.running ?? false}
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
