...
import { ArrowLeft, Sparkles, Brain, ScrollText, Zap, BookOpen, Cpu, Wrench } from 'lucide-react'
...
import { OpenClawToolsPanel } from '@/components/agent/OpenClawToolsPanel'
  ...
  return (
    <div className="relative w-56 flex-shrink-0">
      ...
      >
        ...
        <div className="my-2">
          <AgentAvatar emoji={emoji} theme={theme} status="idle" showBody />
        </div>
        ...
        <div
          ...
          style={{ background: `linear-gradient(to top, ${theme}06, transparent)` }}
        />
      </div>
    </div>
  )
    ...
    <div className="min-h-screen p-8">
      ...
      <div className="flex gap-6 mb-8">
        <div
          ...
          style={{ borderColor: `${selectedAgent.identity.theme || '#6366F1'}30` }}
        >
          <div
          ...
          <div className="relative z-10 flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-4 relative">
                <ModelSelector currentModel={selectedAgent.model} onSelect={handleModelChange} compact />
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="bg-white/3 border border-white/5 p-1 h-auto rounded-xl flex flex-wrap justify-start">
          ...
          <TabsTrigger value="tools" className="data-[state=active]:bg-cyber-purple/15 data-[state=active]:text-white text-white/40 gap-2 rounded-lg">
            <Wrench className="h-4 w-4" /> 工具 / MCP
          </TabsTrigger>
        ...
        <TabsContent value="identity">
          <IdentityForm identity={selectedAgent.identity} onSave={(identity) => updateAgent(selectedAgent.id, { identity })} />
        </TabsContent>

        <TabsContent value="model">
          <div className="cartoon-card p-6">
            <ModelSelector currentModel={selectedAgent.model} onSelect={handleModelChange} />
          </div>
        </TabsContent>

        <TabsContent value="soul">
          <SoulForm soul={selectedAgent.soul} onSave={(soul) => updateAgent(selectedAgent.id, { soul })} />
        </TabsContent>

        <TabsContent value="rules">
          <RulesForm rules={selectedAgent.rules} onSave={(rules) => updateAgent(selectedAgent.id, { rules })} />
        </TabsContent>

        <TabsContent value="skills">
          <SkillsSelector skills={selectedAgent.skills} onSave={(nextSkills) => updateAgent(selectedAgent.id, { skills: nextSkills })} />
        </TabsContent>
